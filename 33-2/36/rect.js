    ...
    rect.prototype = {
        ...
        command: function(aStr){
            switch (aStr[0]){
                ...
                case('TRA'):
                    switch(aStr[1]) {
                        case('LEF'):this.arr.push([this.run, [2]]);       //这里没写好,重复太多
                            if(aStr[2]>1){
        ...
        randomBuild: function(){
            this.wall.randomBuild(this.x,this.y);
        },
        stopBuild: function(){
            this.wall.stopBuild();
        },
        ...
        * */
        find: function(x,y){  //从x,y找mx,my
            ...
            var mx = this.x;
            var my = this.y;
            if(x==mx&&y==my){this.move();return;} //初始坐标与目标坐标一致
            this.wall.init();
